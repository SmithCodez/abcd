...
use loopswap::asset::{AssetInfoRaw, PairInfo, PairInfoRaw};
...
pub struct WasmMockQuerier {
    ...
    loopswap_pair_querier: LoopswapPairQuerier,
}

#[derive(Clone, Default)]
pub struct LoopswapPairQuerier {
    pairs: HashMap<String, PairInfo>,
}

impl LoopswapPairQuerier {
            ...
            QueryRequest::Wasm(WasmQuery::Raw { contract_addr, key }) => {
                ...
                if key.to_vec() == prefix_pair_info {
                    let pair_info: PairInfo =
                        match self.loopswap_pair_querier.pairs.get(contract_addr) {
                            Some(v) => v.clone(),
                            None => {
                                return SystemResult::Err(SystemError::InvalidRequest {
...
impl WasmMockQuerier {
    pub fn new(base: MockQuerier<Empty>) -> Self {
        WasmMockQuerier {
            base,
            loopswap_pair_querier: LoopswapPairQuerier::default(),
        }
    }

    // configure the loopswap pair
    pub fn with_loopswap_pairs(&mut self, pairs: &[(&String, &PairInfo)]) {
        self.loopswap_pair_querier = LoopswapPairQuerier::new(pairs);
    }
