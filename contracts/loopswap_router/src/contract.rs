...
use loopswap::pair::{QueryMsg as PairQueryMsg, SimulationResponse};
use loopswap::querier::query_pair_info;
use loopswap::router::{
...
pub fn instantiate(
    ...
    CONFIG.save(
        deps.storage,
        &Config {
            loopswap_factory: deps.api.addr_canonicalize(&msg.loopswap_factory)?,
        },
    )?;
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        loopswap_factory: deps
            .api
            .addr_humanize(&state.loopswap_factory)?
            .to_string(),
    };
...
fn simulate_swap_operations(
    ...
    let loopswap_factory = deps.api.addr_humanize(&config.loopswap_factory)?;
